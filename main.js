...
const products = [
  {
    id: 1,
    name: 'Mistletoe magic candle',
    img: './img/Products/Mistletoe.jpeg',
    tag: 'CA$49.99',
    price: 49.99,
    quantity: 2
  },
...
function switchBtnStyle () {
  ...
  if (currentStep === 2) {
    nextBtn.innerHTML = 'Confirm Order'
  } else {
...
function cartItemQty (e) {
  if (e.target.classList.contains('item__quantity--btn')) {
    ...
    // priceNode.innerHTML = formatter.format(Number(thisPrice) * Number(thisQty)).toFixed(2) 
    // alert(Number(thisPrice)* Number(thisQty));
    priceNode.innerHTML = formatter.format((Number(thisPrice) * Number(thisQty)).toFixed(2));
  }
  totalAmount()
}
...
function deliveryFee (e) {
  if (e.target.type === 'radio') {
    ...
    if (fee === 0) {
      cart.querySelector('.cart__shipping--fee').innerHTML = 'Free'
      currentShippingFee = 0
    } else if (fee === 20.00) {
      cart.querySelector('.cart__shipping--fee').innerHTML = '$20.00'
      currentShippingFee = 20.00
    }
    totalAmount()
  }
}
...
// 8. 送出結帳
function checkOut () {
  alert('The total amount is  ' + cart.querySelector('.cart__sum--price').innerHTML + ' Thank you for visiting!')
}
